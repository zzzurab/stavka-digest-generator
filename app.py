...
def scrape(url):
    ...
    # Always render the MAIN match page before extracting kickoff time.
    # A plain requests response can contain a stale/UTC time (e.g. 18:00),
    # while the visible Stavka TV page shows the correct Moscow time (e.g. 21:00).
    # Team-page rendering above is only for logo lookup and must not be used
    # as the source of the match datetime.
    main_render = render_stavka_page(url)
    main_rendered = main_render.get('html', '')
    main_visible = main_render.get('visible', {})
    if main_rendered:
