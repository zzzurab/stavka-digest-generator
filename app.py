...
def scrape(url):
    ...
    if team1 and team2:
        logo1, logo2 = stavka_team_logos(soup, team1, team2, url)
    ...
    if not (logo1 and logo2):
        ...
        if rendered:
            ...
            logo1, logo2 = stavka_team_logos(rsoup, team1, team2, url)

    if not team1 or not team2:
        raise ValueError('Не удалось определить команды')

    # If the first soup was the raw response and the rendered page exists,
    # use the rendered DOM for the remaining page fields too.
    data_soup = BeautifulSoup(rendered, 'html.parser') if rendered else soup

    league = extract_league(data_soup)
    datetime_text, time, date = extract_match_datetime(
        data_soup, team1, team2, url
    )
...
$('#go').onclick=async()=>{const urls=$('#urls').value.split(/\n+/).map(x=>x.trim()).filter(Boolean);if(!urls.length)return;$('#go').disabled=true;$('#status').textContent='Собираю актуальные данные со страниц…';data=[];for(const url of urls){try{let r=await fetch('/api/scrape?url='+encodeURIComponent(url));let j=await r.json();if(j.error)throw Error(j.error);data.push(j);data.sort((a,b)=>{const parse=x=>{const m=(x.date||'').match(/(\d{1,2})\s+([а-яё]+)/i);const t=(x.time||'').match(/(\d{1,2}):(\d{2})/);const months={'января':1,'февраля':2,'марта':3,'апреля':4,'мая':5,'июня':6,'июля':7,'августа':8,'сентября':9,'октября':10,'ноября':11,'декабря':12};return m&&t?[months[m[2].toLowerCase()]||99,+m[1],+t[1]*60+ +t[2]]:[99,99,9999]};const A=parse(a),B=parse(b);return A[0]-B[0]||A[1]-B[1]||A[2]-B[2]});render();save()}catch(e){$('#results').insertAdjacentHTML('beforeend',`<div class="row error"><b>Не удалось обработать:</b> ${esc(url)}<br>${esc(e.message)}</div>`)}}save();$('#go').disabled=false};$('#html').onclick=async()=>{if(data.length<15&&!confirm('В письме меньше 15 матчей. Всё равно сформировать HTML?'))return;let r=await fetch('/api/html',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({matches:data})});let blob=await r.blob();let a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='digest_generated.html';a.click();URL.revokeObjectURL(a.href)};$('#copy').onclick=()=>navigator.clipboard.writeText($('#json').value);</script></body></html>'''
