    ...
    try:
        with sync_playwright() as pw:
            ...
            visible = page.evaluate("""() => {
                ...
                const imgs = [...document.images].filter(img => { const src = img.currentSrc || img.src || ''; const r = img.getBoundingClientRect(); return /static\.stavka\.tv\/upload\/team\//i.test(src) && r.width > 0 && r.height > 0; }).map(img => ({src: img.currentSrc || img.src, alt: clean(img.alt), title: clean(img.title)}));
...
def scrape(url):
    ...
    if (not logo1 or not logo2) and visible.get('imgs'):
        ...
        def by_name(name):
            n = clean(name).lower()
            return next((x.get('src','') for x in imgs if n and n in (x.get('alt','') + ' ' + x.get('title','')).lower()), '')
        logo1 = logo1 or by_name(team1)
        logo2 = logo2 or by_name(team2)
        if not logo1 and len(imgs) >= 2: logo1 = imgs[0].get('src','')
        if not logo2 and len(imgs) >= 2: logo2 = imgs[1].get('src','')
