...
def render_stavka_page(url):
    """Render the Vue page so client-generated <img> tags become available.
    Uses an installed Google Chrome first, then Playwright Chromium if present.
    """
    try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            try:
                page.locator("img[src*='static.stavka.tv/upload/team/']").first.wait_for(state='attached', timeout=6000)
            except Exception:
                pass
...
def scrape(url):
    ...
    if soup is None:
        rendered = render_stavka_page(url)
        if not rendered:
            raise ValueError(
    ...
    # Only use team images from static.stavka.tv/upload/team/.
    if team1 and team2:
        s_logo1, s_logo2 = stavka_team_logos(soup, team1, team2, url)
        logo1 = s_logo1 or logo1
        logo2 = s_logo2 or logo2
    ...
    if not (logo1 and logo2):
        ...
        if rendered:
            ...
            s_logo1, s_logo2 = stavka_team_logos(rsoup, team1, team2, url)
            logo1 = s_logo1 or logo1
            logo2 = s_logo2 or logo2
    ...
    # Keep raw HTML as the primary source for metadata so the server/browser
    # timezone cannot shift the kickoff time. Rendered HTML is for Vue logos.
    data_soup = soup
    league = extract_league(data_soup)
    datetime_text, time, date = extract_match_datetime(data_soup, team1, team2, url)
    sport = infer_sport(url)
    odds, prediction = extract_main_prediction(data_soup)
    if (not time or not odds or not league) and rendered:
        rsoup = BeautifulSoup(rendered, 'html.parser')
        if not league: league = extract_league(rsoup)
        if not time: datetime_text, time, date = extract_match_datetime(rsoup, team1, team2, url)
        if not odds: odds, prediction = extract_main_prediction(rsoup)
    ...
    def do_POST(self):
        if self.path=='/api/scrape_many':
            try:
                ...
                self.send_body(json.dumps(scrape_many(urls),ensure_ascii=False),ctype='application/json; charset=utf-8')
            except Exception as e:
                self.send_body(json.dumps({'error':str(e)},ensure_ascii=False),400,'application/json; charset=utf-8')
            return
        if self.path!='/api/html': self.send_body('Not found',404); return
        try:
            n=int(self.headers.get('Content-Length','0')); payload=json.loads(self.rfile.read(n)); matches=payload.get('matches',[])
